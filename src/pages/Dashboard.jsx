...
export default function Dashboard({ onNavigate }) {
  ...
  useEffect(() => {
    setS(orderDb.getTodaySummary(currentShop.id))
    setLowStock(ingredientDb.getLowStock(currentShop.id))
  }, [currentShop.id])
  ...
  return (
    <div className="page">
      ...
      <div className="grid-2" style={{ marginBottom: 12 }}>
        <div className="stat-card">
          <div className="stat-label">📉 ค่าธรรมเนียม (GP/Ads/คูปอง)</div>
      ...
      {s.subtotal > 0 && (
        <div className="alert alert-success" style={{ borderRadius: 12, marginBottom: 20 }}>
          ...
          <div className="text-sm">
            <span className="font-semibold">ยอดขาย ≠ กำไร</span> — ขายได้ <strong>{fmt(s.subtotal)} ฿</strong> แต่หัก GP/Ads/คูปอง <strong>{fmt(s.totalFees)} ฿</strong> และต้นทุน <strong>{fmt(s.totalCost)} ฿</strong> แล้ว เหลือกำไรจริง <strong>{fmt(s.netProfit)} ฿</strong>
          </div>
        </div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div className="grid-2">
          <button className="btn btn-secondary btn-lg" onClick={() => onNavigate('ingredients')}>
            <Package size={20} /> วัตถุดิบ
          </button>
          <button className="btn btn-secondary btn-lg" onClick={() => onNavigate('menus')}>
            <ShoppingBag size={20} /> เมนู
          </button>
        </div>
      </div>
    </div>
