...
import { Plus, Store, ChevronRight, Pencil } from 'lucide-react'
...
export default function ShopSelector() {
  ...
  function handleSave() {
    if (!name.trim()) return
    if (editShop) {
      updateShop(editShop.id, { name: name.trim(), emoji })
    } else {
      const shop = addShop(name.trim(), emoji)
      selectShop(shop)
      return
    }
    setShowForm(false)
    ...
    <div style={{ minHeight: '100dvh', background: 'linear-gradient(135deg, #FF6B35 0%, #f5a16d 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 40, color: 'white' }}>
          ...
          <h1 style={{ fontSize: '1.8rem', fontWeight: 800 }}>ระบบจัดการต้นทุน</h1>
          <p style={{ opacity: 0.85, marginTop: 6, fontSize: '1rem' }}>เลือกร้านที่ต้องการจัดการ</p>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 16 }}>
          {shops.map(shop => (
            ...
            >
              ...
              <span style={{ flex: 1, textAlign: 'left', fontWeight: 700, fontSize: '1.1rem', color: '#1f2937' }}>
                {shop.name}
              </span>
        ...
        {/* Add shop */}
        <button onClick={openAdd} className="btn btn-lg btn-full" style={{ background: 'rgba(255,255,255,0.2)', color: 'white', border: '2px dashed rgba(255,255,255,0.5)' }}>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  {EMOJIS.map(e => (
                    <button
                      ...
                      onClick={() => setEmoji(e)}
                      style={{
                        fontSize: 32, padding: 10, border: '2px solid',
                        borderColor: emoji === e ? '#FF6B35' : '#e5e7eb',
                        borderRadius: 12, cursor: 'pointer', background: emoji === e ? '#fff0eb' : 'white',
